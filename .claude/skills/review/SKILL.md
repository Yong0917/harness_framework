...
먼저 다음 문서들을 읽어라:
- `/CLAUDE.md` (필수)
- `/docs/ARCHITECTURE.md` (존재하고 `{...}` 플레이스홀더가 없으면 읽는다. 빈 템플릿이면 skip.)
- `/docs/ADR.md` (존재하고 `{...}` 플레이스홀더가 없으면 읽는다. 빈 템플릿이면 skip.)
...
5. **빌드/테스트 통과**: CLAUDE.md의 `## 명령어` 섹션에 정의된 테스트 커맨드를 실행하여 에러 없이 통과하는가?
